(() => {
    ...
    return `<div class="bank-v5-shell" data-bank-v6>
      <section class="bank-v5-panel" id="banki-bankomati" data-bank-v6-panel="atms">
        <div class="bank-v5-hero">
          ...
          <div class="bank-v5-statline">
            <div><strong>15</strong><span>банкомата</span></div>
            <i></i>
            <div><strong>13</strong><span>адреса</span></div>
            <i></i>
        ...
        <div class="bank-v5-trust">
          <span>✓</span>
          <p><strong>Само проверими данни.</strong> Ако 24/7 или внасяне не са надеждно потвърдени, не ги отбелязваме.</p>
        </div>
      ...
      <section class="bank-v5-panel" id="banki-ofisi" data-bank-v6-panel="offices" hidden>
        <div class="bank-v5-hero bank-v5-hero--office">
          ...
          <div class="bank-v5-statline bank-v5-statline--single">
            <div><strong>5</strong><span>офиса</span></div>
          </div>
        </div>
  ...
  function applyV6(root){
    if (!root || root.dataset.bankV6Applied === "true") return;
    root.dataset.bankV6Applied = "true";
  ...
  function init(){
    ...
    const root = document.querySelector('[data-info-category-root="banki"]');
    if (!root) return;

    // The existing page has three legacy render stages. Wait until the last one
    // finishes, then take ownership once so nothing can overwrite the new UI.
    const legacyFinished = () =>
      !!root.querySelector('#banki-sections[data-enhanced="true"]') &&
      !root.querySelector('.info-loading');
    ...
    const observer = new MutationObserver(() => {
      if (!legacyFinished()) return;
      observer.disconnect();
      requestAnimationFrame(() => applyV6(root));
    });
    observer.observe(root, {childList:true, subtree:true, attributes:true, attributeFilter:['data-enhanced']});
  }
