(() => {
  ...
  const PUBLIC_CATEGORIES = Object.freeze([
    ...
    Object.freeze({ id: "zavedenia", label: "Заведения", searchLabel: "Заведения", description: "Ресторанти, кафенета, пицарии и доставки.", route: "zavedenia.html", values: Object.freeze({ business: "Заведения", question: "Заведения" }) }),
    Object.freeze({ id: "rabota", label: "Услуги", searchLabel: "Услуги", description: "Услуги, предлагани от местни хора и фирми.", route: "rabota.html", values: Object.freeze({ business: "Работа и услуги", question: "Работа и услуги", listing: "Услуги" }) }),
    Object.freeze({ id: "obyavi", label: "Обяви", navigationLabel: "Всички обяви", searchLabel: "Обяви", description: "Купува, продава, подарява, наема и търси.", route: "obyavi.html", values: Object.freeze({ question: "Обяви" }) }),
    Object.freeze({ id: "sabitiya", label: "Събития", searchLabel: "Събития", description: "Предстоящи и актуални събития в Лом.", route: "sabitiya.html", values: Object.freeze({ question: "Събития и град" }) })
  ]);
  ...
  function staticSearchRecords() {
    return PUBLIC_CATEGORIES.map((item) => ({ type: "Категория", title: item.searchLabel || item.label, desc: item.description, url: item.route }));
  }
  ...
  function populateCategorySelect(select) {
    ...
    const categories = source === "business" ? BUSINESS_CATEGORY_IDS.map((id) => byId.get(id)).filter(Boolean) : PUBLIC_CATEGORIES;
    replaceOptions(select, categories.map((item) => ({ value: item.values?.[source], label: item.label })).filter((item) => item.value), source === "business" ? "Избери" : "Избери категория");
  }
  ...
  function observePublicCategoryLabels() {
    const targets = [
      document.querySelector("#businesses-list"), document.querySelector("#home-businesses"),
      document.querySelector("#business-detail-category-tag"), document.querySelector("#business-detail-category"),
    ...
    const scheduleSync = () => {
      if (queued) return;
      queued = true;
      queueMicrotask(() => { queued = false; syncPublicCategoryLabels(); });
    };
