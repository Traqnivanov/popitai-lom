// Попитай.Лом — валидация на формата за обяви + Stage 1 taxonomy
(() => {
  "use strict";
  ...
  const fields = {
    ...
    subcategory: document.querySelector("#listing-subcategory"),
  ...
  if (!fields.title || !fields.category || !fields.subcategory || !fields.description || !fields.phone) return;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .listing-field-meta{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-top:6px;color:#667085;font-size:13px;line-height:1.45}
      .listing-field-hint{flex:1}.listing-field-counter{flex:0 0 auto;white-space:nowrap;font-variant-numeric:tabular-nums;font-weight:700}
      .listing-field-counter.is-near-limit{color:#9a6700}.listing-field-counter.is-over-limit{color:#b42318}
      .listing-field-error{min-height:0;margin:6px 0 0;color:#b42318;font-size:13px;font-weight:700;line-height:1.4}
      .listing-field-error:empty{display:none}
      .content-form input.is-invalid,.content-form textarea.is-invalid,.content-form select.is-invalid{border-color:#b42318!important;box-shadow:0 0 0 3px rgba(180,35,24,.12)!important}
      .content-form input.is-valid,.content-form textarea.is-valid,.content-form select.is-valid{border-color:#2e7d32}
    `;
  ...
  function ensureSupport(field, { hint = "", max = 0 } = {}) {
    ...
    return { error, counter: max ? document.querySelector(`#${baseId}-counter`) : null, max };
  }

  injectStyles();

  const support = {
    title: ensureSupport(fields.title, { hint: "Минимум 5, максимум 120 знака.", max: 120 }),
    category: ensureSupport(fields.category),
    subcategory: ensureSupport(fields.subcategory),
    listingType: fields.listingType ? ensureSupport(fields.listingType) : null,
    description: ensureSupport(fields.description, { hint: "Минимум 20 знака — опиши подробно.", max: 5000 }),
    phone: ensureSupport(fields.phone, { hint: "Например: 0876 936 184 или +359 876 936 184" })
  };
  ...
  function validateCategory(value) {
    ...
    if (!category) return "Избери категория.";
    const allowed = window.PopitaiCategoryDictionary?.listingCategories || [];
    if (allowed.length && !allowed.includes(category)) return "Избери валидна категория.";
    return "";
  }
  ...
  function validateListingType(value) {
    ...
    return String(value || "").trim() ? "" : "Избери тип обява.";
  }
  ...
  function validatePhone(value) {
    ...
    if ((s.match(/\+/g) || []).length > 1 || (s.includes("+") && !s.startsWith("+"))) return "Знакът + може да бъде само веднъж и в началото.";
  ...
  function updateCounter(key) {
    ...
    if (!item?.counter || !item.max) return;
    const len = textLength(fields[key]?.value);
    item.counter.textContent = `${len} / ${item.max}`;
    item.counter.classList.toggle("is-near-limit", len >= item.max * 0.9 && len <= item.max);
    item.counter.classList.toggle("is-over-limit", len > item.max);
  }
  ...
  Object.keys(fields).forEach((key) => {
    ...
    field.addEventListener(eventName, () => {
      updateCounter(key);
      if (field.dataset.touched === "true" || field.classList.contains("is-invalid")) validateField(key, { showSuccess: true });
      if (key === "category" && fields.subcategory) {
        if (fields.subcategory.dataset.touched === "true" || fields.subcategory.classList.contains("is-invalid")) {
          validateField("subcategory", { showSuccess: true });
        }
      }
    });
  });
  ...
  function validateForm() {
    const order = ["title", "category", "subcategory", "listingType", "description", "phone"];
    order.forEach((key) => {
      if (fields[key] && !fields[key].disabled) {
    ...
    const firstInvalidIndex = results.findIndex((value) => !value);
