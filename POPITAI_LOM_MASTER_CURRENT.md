...
Първият екран не показва цялата taxonomy. Води с търсене `Каква услуга търсиш?`, няколко конкретни бързи задачи и девет ясни семейства. След owner consolidation verdict от **Work 2 · 11.09.2026** и допълнението за пътнически превоз от **Work 2 · 13.09.2026** видимите discovery входове са 46:
...
2. **Почистване и поддръжка** — почистване; пране на мека мебел и килими; двор, градина и озеленяване; борба с вредители.
3. **Автомобилни услуги** — автосервиз; диагностика; гуми; автоелектро и автоклиматици; автомивка и детайлинг; пътна помощ.
4. **Транспорт, преместване и доставки** — товарен транспорт; хамали и преместване; доставки; пътнически превоз.
5. **Красота и лична грижа** — фризьор и бръснар; маникюр и педикюр; козметика и грим; немедицински масаж.
6. **Грижа за хора и животни** — детегледачки; грижа за възрастни; помощ в дома; гледане и разхождане на домашни любимци; грижа и подстригване на домашни любимци.
7. **Обучение, уроци и спорт** — уроци и курсове; шофьорски курсове; спорт и танци.
8. **Техника, дигитални и професионални услуги** — ремонт на техника; ИТ, сайтове и дизайн; счетоводство; правни услуги; преводи.
9. **Събития и творчески услуги** — фото и видео; DJ и музика; декорация; кетъринг; организация на събития.

Това е consolidation на публичния discovery слой, не destructive taxonomy migration:

- `Почистване на дом` и `Офиси и входове` са филтри на `Почистване`;
- `Двор и градина` и `Озеленяване` са филтри на `Двор, градина и озеленяване`;
- `Домашна помощ` и `Домашни помощници` стават един вход `Помощ в дома`, откриваем и чрез cross-link от `Почистване и поддръжка`;
- `Товарен транспорт` и `Транспорт с бус/камион` стават `Товарен транспорт`;
- `Хамали` и `Преместване` стават `Хамали и преместване`;
- `Пътнически превоз` е отделен marketplace вход за частни оферти на превозвачи, включително пътувания в чужбина; не дублира проверените контакти, разписания и транспортна информация в `Info Лом`;
- `Козметични услуги` и `Грим` стават `Козметика и грим`;
- `Уроци`, `Езици`, `Професионално обучение` и `Компютърни курсове` стават филтри на `Уроци и курсове`;
- `Компютри/лаптопи` и `Телефони/електроника` стават филтри на `Ремонт на техника`;
- `IT`, `Сайтове` и `Дизайн` стават филтри на `ИТ, сайтове и дизайн`;
- `Фото` и `Видео` стават филтри на `Фото и видео`;
- `Други професионални услуги` не е отделна visible card; остава form/search fallback.

Старите точни понятия се пазят като контролирани search aliases, филтри и backward-compatible discovery context. Обединяването не изтрива, не прехвърля и не променя lifecycle owner на реален запис, не създава нова форма и не променя persisted Listing taxonomy без отделния production checkpoint.
...
- собствен leaf знак е задължителен, когато family/category знакът би бил неточен или подвеждащ; споделяне е допустимо само при еднакъв визуален смисъл, а видимият текст не се премахва;
- иконите са контролирано изключение от общата цел за максимална лекота: допуска се умерено по-детайлен или малко по-голям SVG, когато опростяването доказано влошава качеството, естетиката или разпознаваемостта;
- изключението не разрешава тежки растерни assets, embedded изображения, ненужни ефекти, неоптимизирани paths или зареждане на цяла неизползвана icon библиотека;
- оптимизацията се прави след визуалното одобрение и се приема само ако desktop, 390 px, 20–24 px и social сравнението не показват влошаване;
- всяко ново icon решение пази Work номер, дата, owner verdict и приложим evidence/SHA в централния регистър; старо решение не се приписва или заменя по памет;
...
| Services taxonomy | 9 семейства и 46 видими discovery входа; 45-те от consolidation решението плюс отделно owner-approved `Пътнически превоз`; старите понятия се пазят като aliases/filters/cross-links без data migration | 59 понятия като 59 отделни видими подкатегории и потенциално 59 отделни icon cards |
...
- Services discovery consolidation: 9 families / 46 visible entries след owner добавянето на `Пътнически превоз` от Work 2 · 13.09.2026; exact aliases/filters и bounded exact-choice flow са приложени в review prototype; production persistence/migration не е променяна;
