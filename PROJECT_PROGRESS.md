...
### Въпроси
- Общата страница `vaprosi.html` вече използва отделен Supabase renderer `questions-public-v1.js`; старият `localStorage` renderer в `script.js` не е собственик на публичния списък.
...
Следните открити точки вече са изчистени:

1. **Събития — публичен event flow** — има отделен публичен renderer за одобрени предстоящи `events`.
...
3. **Образование и култура — signal routing** — сигналът използва каноничната категория `obrazovanie`, която съответства на Info Lom/Admin routing-а.
...
5. **„Майстори и ремонти“ — въпроси** — старият localStorage render за този публичен блок е изключен; използва се Supabase, без промяна на search priority, Иванов Ремонти, фирми, обяви, роли, лимити или Admin.

### Оставащи доказани точки преди финалния browser/device QA

1. **Институции — render ownership.**
   - Текущата страница още зарежда няколко последователни слоя: generic/canonical/approved-extension/final/enhancement.
   - Не трябва да се премахват на сляпо, защото носят вече одобрено специфично съдържание и UX.
   - Нужен е behavior-preserving consolidation до един owner, без загуба на съдържание, CTA, официални връзки или signal flow.

2. **Магазини — пълна field-specific validation.**
   - Телефонът има собствена semantic validation.
   - Останалите задължителни полета още разчитат основно на browser `reportValidity()` и трябва да се приведат към общото правило: грешка до конкретното поле, blur validation, live correction и фокус към първата грешка.
...
- Изчистване на render ownership при „Институции“ с един собственик и без промяна на одобреното съдържание/UX.
- Довършване на field-specific validation на формата „Добави магазин“.
- След тези две точки: production browser/device QA — desktop + mobile, реални бутони, форми, modal/focus, login states, console/runtime, cache/load-order/deployment.
