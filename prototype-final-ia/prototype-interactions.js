...
(() => {
  ...
  function syncPriceContext(form,{reset=false}={}){
    const category=form?.querySelector('#listing-category')?.value||'',type=form?.querySelector('#listing-type')?.value||'',price=form?.querySelector('#listing-price'),label=form?.querySelector('[data-price-label]'),fieldset=form?.querySelector('[data-price-options]'),period=form?.querySelector('#work-compensation-period'),periodField=form?.querySelector('[data-work-period-field]');
    if(!price||!label||!fieldset)return;const context=formOwners.listingPriceContext(category),previousContext=fieldset.dataset.priceContext||'',preserveWorkNegotiable=category==='Работа'&&!reset&&previousContext==='work'&&Boolean(form.querySelector('#price-negotiable')?.checked);
    if(reset){price.value='';price.disabled=false;if(period){period.value='';period.disabled=category!=='Работа';}}
    label.textContent=category==='Работа'?formOwners.workCompensationLabel(type):context.label;fieldset.dataset.priceContext=context.key;fieldset.hidden=!context.options.length;
    ...
    const negotiable=form.querySelector('#price-negotiable');if(preserveWorkNegotiable&&negotiable)negotiable.checked=true;
    if(periodField)periodField.hidden=category!=='Работа';
    if(period){if(category!=='Работа'){period.value='';period.disabled=true;}else period.disabled=Boolean(negotiable?.checked);}
    if(category==='Работа')price.disabled=Boolean(negotiable?.checked);
    const error=fieldset.querySelector('#price-state-error');if(error)error.textContent='';
  }
  function syncPriceState(form,target=null){
    const category=form?.querySelector('#listing-category')?.value||'',price=form?.querySelector('#listing-price'),period=form?.querySelector('#work-compensation-period'),neg=form?.querySelector('#price-negotiable'),free=form?.querySelector('#price-free'),onRequest=form?.querySelector('#price-on-request');if(!price)return;
    if(category==='Работа'){
      if(target===neg&&neg?.checked){price.value='';if(period)period.value='';price.disabled=true;if(period)period.disabled=true;form.dataset.workPeriodLegacy='false';return;}
      if(target===price&&price.value.trim()){if(neg)neg.checked=false;price.disabled=false;if(period)period.disabled=false;form.dataset.workPeriodLegacy='false';return;}
      if(target===period){if(period?.value&&neg)neg.checked=false;price.disabled=Boolean(neg?.checked);if(period)period.disabled=Boolean(neg?.checked);form.dataset.workPeriodLegacy='false';return;}
      price.disabled=Boolean(neg?.checked);if(period)period.disabled=Boolean(neg?.checked);return;
    }
  ...
  function favoriteItemFrom(button){
    if(!button)return null;
    const type=button.dataset.favoriteType||'',id=button.dataset.favoriteRecordId||'',title=button.dataset.favoriteTitle||'',href=button.dataset.favoriteHref||'';
    if(!favoriteEligible.has(type)||!id||!title||!href)return null;
    return {key:`${type}:${id}`,type,id,title,href};
  }
  function syncFavoriteButton(button){
    const item=favoriteItemFrom(button);if(!item)return;
    const saved=favoriteSaved.has(item.key),label=button.querySelector('[data-favorite-label]');
    button.dataset.favoriteKey=item.key;button.setAttribute('aria-pressed',String(saved));button.setAttribute('aria-label',saved?`Премахни „${item.title}“ от Любими`:`Запази „${item.title}“`);if(label)label.textContent=saved?'Запазено':'Запази';
  }
  function announceFavorite(button,message){
    const live=button.closest('[data-favorite-utility]')?.querySelector('[data-favorite-live]');if(!live)return;live.textContent='';requestAnimationFrame(()=>{live.textContent=message;});
  }
  function showFavoriteLogin(item){
    pendingFavoriteIntent=item;let note=document.querySelector('.favorite-login-note');if(!note){note=document.createElement('div');note.className='favorite-login-note';note.setAttribute('role','status');document.body.append(note);}note.innerHTML='<span>Влез, за да запазиш този запис</span><a href="#profile?favoriteIntent=1">Вход</a>';
  }
  function augmentFavorites(){
    document.querySelectorAll('#app-main [data-favorite-toggle][data-favorite-record-id][data-favorite-type]').forEach(syncFavoriteButton);
    if(favoriteReturnFocusKey){const target=[...document.querySelectorAll('#app-main [data-favorite-toggle]')].find(button=>favoriteItemFrom(button)?.key===favoriteReturnFocusKey);favoriteReturnFocusKey='';target?.focus({preventScroll:true});}
  }
  function favoriteGroups(){
    if(!favoriteSaved.size)return '<article class="empty-card"><h2>Нямаш запазени записи</h2><p>Когато запазиш конкретен публичен запис, той ще се появи тук.</p></article>';
    const groups=new Map();favoriteSaved.forEach(item=>{if(!groups.has(item.type))groups.set(item.type,[]);groups.get(item.type).push(item);});
    return [...groups.entries()].map(([type,items])=>`<section class="saved-group"><h2>${esc(favoriteLabels[type]||type)}</h2><div class="result-list">${items.map(item=>`<article class="result-row saved-entry"><div><a class="saved-entry-link" href="${esc(item.href)}"><h3>${esc(item.title)}</h3></a><p>${esc(favoriteLabels[item.type]||item.type)}</p></div><button class="btn soft" type="button" data-favorite-remove="${esc(item.key)}" aria-label="Премахни „${esc(item.title)}“ от Любими">Премахни</button></article>`).join('')}</div></section>`).join('');
  }
  function profileMarkup(){
    const pending=pendingFavoriteIntent;
    if(!favoriteLoggedIn)return `<div class="page favorites-profile" data-favorites-profile>${pageHead('Профил',pending?'Влез, за да запазиш този запис':'Влез, за да виждаш собственото съдържание и запазените записи.')}<div class="shell"><article class="profile-card"><div><h2>${pending?'Влез, за да запазиш този запис':'Не си влязъл в профила си'}</h2><p>${pending?`След демонстрационния вход ще се върнеш към „${esc(pending.title)}“ и записът ще бъде запазен.`:'За да използваш „Любими“, е необходим профил.'}</p></div><button class="btn primary" type="button" data-favorite-demo-login>Вход</button></article><section class="saved-section"><h2>Запазени</h2><p>След вход тук се показват запазените записи, групирани по вид.</p></section><p class="favorite-session-note">В този прототип няма реален вход и няма постоянно съхранение.</p></div></div>`;
    return `<div class="page favorites-profile" data-favorites-profile>${pageHead('Профил','Твоето съдържание и запазените записи.')}<div class="shell"><article class="profile-card"><div><h2>Профил</h2><p>Демонстрационно състояние след вход.</p></div><button class="btn soft" type="button" data-favorite-demo-logout>Изход</button></article><section class="saved-section"><div class="section-head"><div><h2>Запазени</h2><p>Групирани по вид съдържание.</p></div></div>${favoriteGroups()}</section><p class="favorite-session-note">Запазването е само за текущата отворена сесия на прототипа и не се записва в Supabase.</p></div></div>`;
  }
  ...
  function completeDemoLogin(){
    favoriteLoggedIn=true;
    if(pendingFavoriteIntent){const item=pendingFavoriteIntent;favoriteSaved.set(item.key,item);favoriteReturnFocusKey=item.key;pendingFavoriteIntent=null;routeTo(item.href);return;}
    rerenderProfile();
  }
  ...
  function handleClick(event){
    ...
    if(event.target.closest('[data-favorite-demo-login]')){completeDemoLogin();return;}if(event.target.closest('[data-favorite-demo-logout]')){favoriteLoggedIn=false;favoriteSaved.clear();pendingFavoriteIntent=null;rerenderProfile();return;}
    const favorite=event.target.closest('[data-favorite-toggle]');if(favorite){const item=favoriteItemFrom(favorite);if(!item)return;if(!favoriteLoggedIn){showFavoriteLogin(item);return;}const removed=favoriteSaved.has(item.key);if(removed)favoriteSaved.delete(item.key);else favoriteSaved.set(item.key,item);syncFavoriteButton(favorite);announceFavorite(favorite,removed?'Премахнато от Любими.':'Запазено в Любими.');return;}
  ...
  function handleChange(event){const target=event.target,form=target.closest?.('[data-proto-form]');if(target.matches('[data-demo-upload]')){acceptUpload(target);if(form)form.dataset.dirty='true';return;}if(target.id==='listing-category'){if(form)form.dataset.workPeriodLegacy='false';syncListingForm({preserve:false,resetDiscovery:true});}if(target.id==='listing-subcategory'){if(form?.querySelector('#listing-category')?.value==='Работа')form.dataset.discoveryContext=target.value||'';syncListingForm({preserve:true});}if(target.id==='listing-type')syncListingForm({preserve:true});if(target.id==='shop-category')syncShopTags();if(target.closest?.('[data-proto-form][data-form-kind="question"]'))syncQuestionHints();if(form){form.dataset.dirty='true';if(['price-free','price-negotiable','price-on-request','work-compensation-period'].includes(target.id))syncPriceState(form,target);validators.validateControl(form,target);}}
  function handleInput(event){const target=event.target,form=target.closest?.('[data-proto-form]');if(form)form.dataset.dirty='true';if(form&&target.matches('input,textarea,select'))validators.setError(target,'');if(form?.dataset.formKind==='listing'&&target.matches('[data-other-service-text]')){const exact=target.value.trim(),title=form.querySelector('[name="Заглавие"]'),description=form.querySelector('[name="Описание"]');if(exact&&title&&!title.value)title.placeholder=`Предлагам ${exact} в Лом`;if(exact&&description&&!description.value)description.placeholder=`Опиши „${exact}“, район, срок и важни условия.`;}if(form?.dataset.formKind==='listing'&&target.id==='listing-price'){if(form.querySelector('#listing-category')?.value==='Работа')form.dataset.workPeriodLegacy='false';syncPriceState(form,target);}}
  ...
  function closeTransient(){closeMoreMenu();closeAdd({restoreFocus:false});closeShareOverlay(document.querySelector('[data-share-overlay]:not([hidden])'),{restoreFocus:false});document.querySelector('.favorite-login-note')?.remove();}
