...
(() => {
  ...
  function syncListingForm({preserve=true,resetDiscovery=false}={}){
    ...
    const categoryValue=category.value,previousSub=preserve?subcategory.value:'',subs=contracts.listingSubcategories(categoryValue),nextSub=subs.includes(previousSub)?previousSub:'';
    subcategory.innerHTML=optionHtml(subs,nextSub);subcategory.value=nextSub;subcategoryField.hidden=categoryValue!=='Услуги';subcategory.disabled=categoryValue!=='Услуги';subcategory.required=categoryValue==='Услуги';
  ...
  function handleClick(event){
    ...
    const retry=event.target.closest('[data-retry-state]');if(retry){location.hash=location.hash.replace(/([?&])state=error(&|$)/,'$1').replace(/[?&]$/,'');return;}
