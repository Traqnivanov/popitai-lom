...
(() => {
  ...
  function shops(){
    ...
    const cards=(data.shops||[]).map(item=>`<article class="result-row"><div><h3>${esc(item.title)}</h3><p>${esc(item.description)}</p><div class="result-meta"><span class="badge gold">${esc(item.meta)}</span>${item.href?`<a class="btn soft" href="${esc(item.href)}">Отвори</a>`:''}</div></div>${item.phone?`<a class="btn soft" href="tel:${item.phone.replace(/[^\d+]/g,'')}">Обади се</a>`:''}</article>`).join('');
