...
(() => {
  ...
  function home(){
    return `<section class="hero"><div class="shell hero-grid"><div><span class="eyebrow">Лом и региона</span><h1>Намери каквото ти трябва в Лом</h1><p>Услуги, работа, имоти, обяви, местни фирми и проверена полезна информация — на едно разбираемо място.</p><form class="search-box" data-global-search><input name="q" aria-label="Търсене" placeholder="Напр. ВиК, работа, апартамент, автосервиз…"><button>Търси</button></form><div class="hero-actions"><button class="btn primary" type="button" data-open-add>＋ Публикувай</button><a class="btn ghost" href="#add/question">Не намираш? Задай въпрос</a></div></div><aside class="hero-side"><strong>Как работи</strong><ul><li>Първо търсиш по задача.</li><li>После публикуваш през правилната система.</li><li>Въпросът е резервната стъпка, ако няма готов отговор.</li></ul></aside></div></section>
      <section class="section"><div class="shell"><div class="section-head"><div><h2>Обяви и услуги</h2><p>Шест основни задачи, без да те затрупваме с всички подкатегории.</p></div><a href="#obyavi">Всички →</a></div><div class="grid cols-3">${marketplace.map(card).join('')}</div><a class="protected-entry" href="#maistori"><strong>Майстори и ремонти</strong><small>Намери майстор за ремонти, ВиК, електро, покриви, дограма и други услуги за дома →</small></a><div class="section-head compact-head"><div><h2>Местни специализирани раздели</h2><p>Магазини, заведения и здравни профили.</p></div></div><div class="special-grid">${specialist.map(x=>`<a class="special-card" href="#${x.id}"><span>${x.icon}</span>${x.title}</a>`).join('')}</div></div></section>
      ...
      <section class="section"><div class="shell"><div class="section-head"><div><h2>Нови обяви и услуги</h2><p>Разгледай примерни предложения от основните местни раздели.</p></div><a href="#obyavi">Виж всички →</a></div><div class="result-list">${demoRow('ВиК услуга — примерен резултат','Примерно предложение за ВиК услуга в Лом.','Услуга','#detail/listing?record=listing-vik','Услуги')}${demoRow('Примерна обява за работа','Обява в избраната област на работа.','Работа','#detail/listing?record=listing-work','Предлага работа')}</div></div></section>
      ...
      <section class="section"><div class="shell"><div class="section-head"><div><h2>Въпроси от общността</h2><p>Попитай хората в Лом, когато не намираш готов отговор.</p></div><a href="#vaprosi">Всички въпроси →</a></div>${demoRow('Къде в Лом мога да намеря добър ВиК майстор?','Примерен въпрос от общността.','Въпрос','#detail/question?record=question-community','Общност')}</div></section>`;
  }

  function hub(query){
    const rows=`${demoRow('ВиК — пример','Примерна ВиК услуга в Лом.','Услуга','#detail/listing?record=listing-vik','Услуги')}${demoRow('Кетъринг — пример','Примерна кетъринг услуга в Лом.','Услуга','#detail/listing?record=listing-catering','Услуги')}${demoRow('Работа — пример','Примерна обява в избраната област на работа.','Работа','#detail/listing?record=listing-work','Работа')}`;
  ...
  function masters(){
    ...
    const cards=protectedGroups.map(name=>{
      ...
      return `<article class="family-card masters-card"><h3>${esc(name)}</h3><p>Фирми, обяви и препоръки.</p><div class="masters-actions"><a class="btn soft" href="${find}">Намери майстор</a><a class="btn soft" href="${offer}">Предложи услуга</a><a class="btn soft" href="${seek}">Търся изпълнител</a></div></article>`;
    }).join('');
  ...
  function serviceGroup(query){
    ...
    const modeNotice=addMode?'<div class="notice ok"><strong>Избери конкретна услуга</strong></div><details class="qa-adapter"><summary>QA: избор на услуга</summary><p>Family-level Add остава choose-first и не се представя като canonical leaf. URL договорът и допустимите стойности не са променяни.</p></details>':'';
