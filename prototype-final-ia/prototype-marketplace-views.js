...
(() => {
  ...
  function home(){
    return `<section class="hero"><div class="shell hero-grid"><div><span class="eyebrow">Лом и региона</span><h1>Намери каквото ти трябва в Лом</h1><p>Услуги, работа, имоти, обяви, местни фирми и проверена полезна информация — на едно разбираемо място.</p><form class="search-box" data-global-search><input name="q" aria-label="Търсене" placeholder="Напр. ВиК, работа, апартамент, автосервиз…"><button>Търси</button></form><div class="hero-actions"><button class="btn primary" type="button" data-open-add>＋ Публикувай</button><a class="btn ghost" href="#add/question">Не намираш? Задай въпрос</a></div></div><aside class="hero-side"><strong>Как работи</strong><ul><li>Първо търсиш по задача.</li><li>После публикуваш през правилната система.</li><li>Въпросът е резервната стъпка, ако няма готов отговор.</li></ul></aside></div></section>
      <section class="section"><div class="shell"><div class="section-head"><div><h2>Обяви и услуги</h2><p>Шест основни задачи, без да те затрупваме с всички подкатегории.</p></div><a href="#obyavi">Всички →</a></div><div class="grid cols-3">${marketplace.map(card).join('')}</div><a class="protected-entry" href="#maistori"><span>ЗАПАЗЕН СИЛЕН ВХОД</span><strong>Майстори и ремонти</strong><small>Ремонти, ВиК, електро, покриви, дограма и други услуги за дома →</small></a><div class="section-head compact-head"><div><h2>Местни специализирани раздели</h2><p>Магазини, заведения и здравни профили.</p></div></div><div class="special-grid">${specialist.map(x=>`<a class="special-card" href="#${x.id}"><span>${x.icon}</span>${x.title}</a>`).join('')}</div></div></section>
      ...
      <section class="section"><div class="shell"><div class="section-head"><div><h2>Въпроси от общността</h2><p>Community помощ, когато няма готов отговор; не се представя като проверена Info Lom информация.</p></div><a href="#vaprosi">Всички въпроси →</a></div>${demoRow('Къде в Лом мога да намеря добър ВиК майстор?','Примерен въпрос от общността.','Въпрос','#detail/question?record=question-community','Общност')}</div></section>`;
  }
  ...
  function masters(){
    const protectedGroups=['Цялостни ремонти','Бани и плочки','ВиК','Електро','Покриви','Боядисване','Дограма','Климатици'];
    const cards=protectedGroups.map(name=>{
      const find=serviceResultsHref(name);
      const offer=PopitaiStage2Contracts.contextualAddUrl({context:'Услуги',group:name,owner:'Listings'});
      const seek=PopitaiStage2Contracts.contextualAddUrl({context:'Услуги',group:name,owner:'Listings',type:'Търси'});
      return `<article class="family-card masters-card"><h3>${esc(name)}</h3><p>Фирми, обяви и препоръки.</p><div class="masters-actions"><a class="btn soft" href="${find}">Намери</a><a class="btn soft" href="${offer}">Предложи услуга</a><a class="btn soft" href="${seek}">Търся изпълнител</a></div></article>`;
    }).join('');
    return `<div class="page">${pageHead('Майстори и ремонти','Ремонти, ВиК, електро, покриви, дограма и други услуги за дома.','Обяви и услуги')}<div class="shell"><div class="notice"><strong>Запазен protected entry.</strong><p>Този prototype route представя съществуващата логика на „Майстори и ремонти“: подраздели, местни фирми, активни предложения/търсения и въпроси. Не заменя и не променя production owner-а.</p></div><div class="page-tools"><a class="btn primary" href="#category-subcategories">Намери майстор</a><a class="btn" href="#add/firm?category=Майстори%20и%20ремонти">Добави фирма</a><a class="btn" href="#add/question?category=Майстори%20и%20ремонти">Задай въпрос</a></div><div id="category-subcategories" class="grid cols-3">${cards}</div><div class="grid cols-3 masters-owner-summary"><article class="content-card"><h3>Местни фирми</h3><p>Съществуващите фирмени профили остават отделен owner.</p><a class="btn soft" href="#firmi">Всички фирми</a></article><article class="content-card"><h3>Активни предложения и търсения</h3><p>Service listings остават в Listings owner.</p><a class="btn soft" href="#obyavi">Всички обяви</a></article><article class="content-card"><h3>Последни въпроси</h3><p>Community въпросите остават отделен Q&A слой.</p><a class="btn soft" href="#vaprosi">Всички въпроси</a></article></div></div></div>`;
  }
  ...
  function serviceGroup(query){
    ...
    const items=family.slice(1).map((item,i)=>{
      const href=addMode
        ? PopitaiStage2Contracts.contextualAddUrl({context:'Услуги',group:item,owner:'Listings'})
        : serviceResultsHref(item);
      return `<a class="family-card" href="${href}"><div class="icon">${icons[i%icons.length]}</div><h3>${esc(item)}</h3><p>${addMode?'Избери тази конкретна услуга за формата.':'Разгледай подходящите обяви и предложения.'}</p><small>${addMode?'Избери за публикуване →':'Виж резултатите →'}</small></a>`;
    }).join('');
    const familyAdd=PopitaiStage2Contracts.contextualAddUrl({context:'Услуги',group:name,owner:'Listings'});
    const modeNotice=addMode?'<div class="notice ok"><strong>Първо избери конкретна услуга.</strong><p>Семейството не се представя като canonical Service leaf и формата не показва несвързани опции.</p></div>':'';
    const footer=addMode
      ? `<a class="btn" href="#service-group?group=${encodeURIComponent(name)}">← Към резултатите</a>`
      : `<a class="btn primary" href="${familyAdd}">＋ Публикувай услуга в тази група</a><a class="btn" href="#uslugi">← Всички услуги</a>`;
