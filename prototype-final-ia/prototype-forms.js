...
(() => {
  ...
  function isEdit(query){return query.get('state')==='edit';}
  function editRecord(query){return query.get('record')||'';}
  ...
  function editFieldValue(kind,label,query){
    const value=editValue(kind,label,editRecord(query));
  ...
  function currentForField(kind,label,query){
    if(isEdit(query)) return editFieldValue(kind,label,query);
    ...
    if(label==='Тип обява') return listingCategory(query)==='Услуги'?'Дава':(query.get('type')||'');
    return '';
  }
  function listingTypeOptions(query){
    const category=listingCategory(query);
    if(category!=='Услуги') return contracts.listingTypes(category);
    if(!isEdit(query)) return [{value:'Дава',label:'Предлагам услуга'}];
    const persisted=currentForField('listing','Тип обява',query);
    return persisted==='Търси'?[{value:'Дава',label:'Предлагам услуга'},{value:'Търси',label:'Търся изпълнител (стар запис)'}]:[{value:'Дава',label:'Предлагам услуга'}];
  }
  ...
  function attributesFor(kind,label,type){
    ...
    else if(kind==='firm'&&label==='Име на фирмата') attrs.push('minlength="2"','maxlength="120"');
    else if(type==='textarea') attrs.push('maxlength="5000"');
  ...
  function listingTextHints(category,type,discovery,canonical){
    ...
    if(category==='Услуги') return [`Напр. Предлагам ${exact?exact.toLocaleLowerCase('bg-BG'):'услуга'} в Лом`,`Опиши какво включва „${exact||'услугата'}“, район, срокове и важни условия.`];
  ...
  function renderField(kind,label,type,index,query,edit,listingContext){
    const id=fieldId(kind,label,index), errorId=`${id}-error`, current=currentForField(kind,label,query);
    ...
    if(type==='textarea'){
      ...
      return prefix+`<div class="field"><label for="${id}">${esc(label)}</label><textarea id="${id}" name="${esc(label)}" rows="5" ${required} ${attrs} aria-describedby="${errorId}" placeholder="${esc(hint)}">${esc(edit?editFieldValue(kind,label,query):current)}</textarea>${fieldError(id)}</div>`;
    }
    if(type==='select'){
  ...
  function uploadSection({label,maxFiles,id}){
    ...
    return `<section class="upload-demo" data-upload-section tabindex="-1" aria-labelledby="${id}-title"><div><strong id="${id}-title">${esc(label)}</strong><span data-upload-count>0 / ${maxFiles}</span></div><p>До ${maxFiles} ${maxFiles===1?'файл':'файла'} · JPG, PNG или WebP · до 10 MB на файл.</p><label class="btn upload-button" for="${inputId}">Избери ${maxFiles===1?'файл':'снимки'}</label><input id="${inputId}" type="file" accept="image/jpeg,image/png,image/webp" ${maxFiles>1?'multiple':''} data-demo-upload data-max-files="${maxFiles}" data-max-bytes="10485760" data-allowed-mime="image/jpeg,image/png,image/webp" aria-describedby="${id}-error"><div class="upload-selected" data-upload-selected aria-live="polite"></div><p class="field-error" id="${id}-error" data-upload-error aria-live="polite"></p></section>`;
  }
  function formPage(kind,query){
    const config=formConfig[kind]||formConfig.listing, state=query.get('state'), edit=state==='edit';
    ...
    if(state==='success') return `<div class="page">${pageHead(config.title,'Успешно изпращане.')}<div class="shell form-wrap"><div class="notice ok"><strong>Успешно изпратено.</strong> Формата е приключена и не може да бъде изпратена повторно.</div></div></div>`;
    ...
    const editNote=edit?'<div class="notice"><strong>Редактираш запис.</strong> Попълнените стойности се запазват, ако има поле за корекция.</div>':'';
