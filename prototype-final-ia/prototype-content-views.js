...
function socialPreview(kind,c){
  const config={
    ...
    shop:{title:'Име на магазин — вид магазин в Лом',desc:'Кратко описание и публичен адрес.',image:'Категориална брандирана визуализация'},
    health:{title:'Име — специалност в Лом',desc:'Специалност и публичен адрес/практика.',image:'Health брандирана визуализация'},
    article:{title:c.heading,desc:'Редакционен откъс от статията.',image:'Основна илюстрация → Попитай.Лом'},
    publication:{title:c.heading,desc:'Собствен кратък текст само когато публикацията има собствен постоянен URL.',image:'Собствена или свързана одобрена визуализация'},
    event:{title:'Име на събитие + дата',desc:'Място, час и кратко описание.',image:'Афиш/снимка при наличност → брандиран fallback'},
    question:{title:c.heading,desc:'Категория + „Виж отговорите в Попитай.Лом“',image:'Брандирана Q&A визуализация'},
    info:{title:c.heading,desc:'Проверена информация за Лом + вид контакт.',image:'Info Lom брандирана визуализация'}
  }[kind];
  if(!config) return '';
  return `<details class="content-card" style="margin-top:18px"><summary><strong>Преглед при споделяне</strong></summary><div style="margin-top:12px"><div class="demo-label">SOCIAL PREVIEW — ПРОТОТИП</div><h3>${esc(config.title)}</h3><p>${esc(config.desc)}</p><p class="help"><strong>Снимка:</strong> ${esc(config.image)}</p><p class="help">Използват се само публичната одобрена версия и одобрени изображения. Pending/скрито съдържание не участва.</p></div></details>`;
}
...
function detail(kind){
  const map={
    ...
    shop:{title:'Примерен магазин',type:'Магазин',desc:'Постоянен read-only профил върху съществуващия Shops owner.',heading:'Примерен местен магазин',body:'Основната категория, краткото описание, публичният адрес, работното време и таговете идват от съществуващия Shop запис.',rows:[['Категория','Хранителни'],['Адрес','Лом'],['Какво ще намерят клиентите','Хранителни стоки · Напитки']]},
    ...
    event:{title:'Примерно събитие',type:'Събитие',desc:'Read-only detail surface върху съществуващия Event owner.',heading:'Предстоящо местно събитие',body:'При реален запис тук се показват описанието, организаторът и само актуалната публична информация.',rows:[['Дата и час','12 септември · 18:00'],['Място','Лом']]},
    ...
    health:{title:'Примерен здравен профил',type:'Здраве',desc:'Read-only detail surface върху специализирания Health/Info owner.',heading:'Примерен лекар — специалност',body:'При реален запис тук се виждат owner типът, специалност, кабинет, публичен контакт и последно потвърдена информация.',rows:[['Owner тип','Лекар'],['Специалност','Примерна специалност'],['Район','Лом'],['Последно потвърдено','Показва се при реален запис']]},
