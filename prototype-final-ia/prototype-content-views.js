...
(() => {
  ...
  function iconCheckpoint(){
    const candidates=[
      {label:'Услуги',file:'wrench-duotone.svg',socialFile:'broom-duotone.svg',key:'services',reason:'Практическа услуга или помощ',avoid:'не е Работа; конкретното съдържание получава по-точен знак',eyebrow:'УСЛУГА · ПОЧИСТВАНЕ',title:'Почистване на дом в Лом'},
      ...
      {label:'Проверена информация',file:'seal-check-duotone.svg',key:'verified',reason:'Потвърден запис с източник и дата',avoid:'не е публикация от общността',eyebrow:'ИНФО ЛОМ · ПРОВЕРЕНО',title:'Работно време и контакти'}
    ];
    const specificThemes=[
      {label:'Почистване',file:'broom-duotone.svg',key:'cleaning'},
      {label:'ВиК',file:'pipe-duotone.svg',key:'plumbing'},
      {label:'Електро',file:'lightning-duotone.svg',key:'electrical'},
      {label:'Боядисване',file:'paint-roller-duotone.svg',key:'painting'},
      {label:'Покриви и дом',file:'house-line-duotone.svg',key:'roofing'},
      {label:'Транспорт',file:'truck-duotone.svg',key:'transport'},
      {label:'Фото',file:'camera-duotone.svg',key:'photo'},
      {label:'Кетъринг',file:'fork-knife-duotone.svg',key:'catering'},
      {label:'Институции',file:'bank-duotone.svg',key:'institutions'},
      {label:'Изгубено животно',file:'paw-print-duotone.svg',key:'lost-animal'}
    ];
    const icon=(item,className='',file=item.file)=>`<span class="${className}" aria-hidden="true"><img src="icons/${file}" alt=""></span>`;
    return `<div class="page icon-system-page">${pageHead('Икони — първи строг визуален checkpoint','Петте най-трудни понятия, проверени в реален малък размер и в тематична social карта без снимка.')}<div class="shell"><div class="icon-system-boundary"><strong>Нищо тук още не заменя иконите в сайта.</strong><span>Одобряват се една по една. Един неодобрен символ спира целия комплект.</span></div><section aria-labelledby="icon-review-title"><div class="section-head"><div><h2 id="icon-review-title">Разпознаваемост в сайта</h2><p>Един стил, тъмносин знак и лек тематичен акцент — без emoji и без детски 3D рисунки.</p></div></div><div class="icon-review-grid">${candidates.map(item=>`<article class="icon-review-card" data-icon-key="${item.key}"><div class="icon-review-heading">${icon(item,'icon-review-emblem')}<div><h3>${item.label}</h3><p>${item.reason}</p></div></div><div class="icon-size-proof" aria-label="Проверка в малък размер">${icon(item,'icon-size-24')}<span>${item.label}</span></div><small><strong>Граница:</strong> ${item.avoid}.</small></article>`).join('')}</div></section><section class="icon-theme-review" aria-labelledby="icon-theme-title"><div class="section-head"><div><h2 id="icon-theme-title">По-конкретната тема има предимство</h2><p>Тези знаци се използват, когато точната тема е известна. Общата категория остава само fallback.</p></div></div><div class="icon-theme-grid">${specificThemes.map(item=>`<article class="icon-theme-card" data-icon-key="${item.key}">${icon(item,'icon-theme-emblem')}<strong>${item.label}</strong></article>`).join('')}</div></section><section class="icon-social-review" aria-labelledby="icon-social-title"><div class="section-head"><div><h2 id="icon-social-title">Конкретният знак при споделяне без снимка</h2><p>Social картата използва най-точната известна тема: например „Почистване“ получава метла, а не общия знак „Услуги“. При одобрена реална снимка се използва снимката.</p></div></div><div class="icon-social-grid">${candidates.map(item=>`<article class="icon-social-card" data-icon-key="${item.key}" aria-label="Social пример за ${item.label}"><div class="icon-social-copy"><span>${item.eyebrow}</span><strong>${item.title}</strong><small>Лом и региона</small></div>${icon(item,'icon-social-emblem',item.socialFile||item.file)}<b>Попитай.Лом</b></article>`).join('')}</div></section><div class="icon-system-decision"><strong>Критерий за приемане</strong><p>Символът трябва да се разпознава за секунда на телефон, да не се бърка със съседна категория и да изглежда убедително и при 1200×630 social композиция.</p></div></div></div>`;
  }
