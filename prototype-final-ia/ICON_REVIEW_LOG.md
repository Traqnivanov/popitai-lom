...
| `TOM1-T002` | Цялостни ремонти / Майстори и ремонти | SHARED broad-renovation asset: ясен 3D вътрешен ъгъл/част от стая в ремонт, две стени + под, с чук и бояджийски валяк; без demolition/rubble и без house/property silhouette | `ACCEPTED` | ACCEPTED / MASTER READY | `popitai_lom_cqlostni_remonti_MASTER_1024.png` — OWNER-accepted chat asset; verified 1024×1024 PNG RGBA with real alpha | OWNER прие визуалната и семантичната посока. MASTER е технически проверен: exact 1024×1024, true alpha, transparent corners, no crop, occupied area 70.5% width / 75% height, safe margins 14.75% left/right and 12.5% top/bottom. Asset-ът е SHARED за `Цялостни ремонти` и `Майстори и ремонти`; следващата разрешена стъпка е exact 128×128 site WebP + 512×512 social WebP от същия master, без redraw. |
