...
(async()=>{
  ...
  async function fresh(type='Предлага работа'){
    const hash='#add/listing?category='+encodeURIComponent('Работа')+'&type='+encodeURIComponent(type);
    const target=win.PopitaiRouter.canonicalHash(hash);
    if(win.PopitaiRouter.getState?.().hash===target){win.PopitaiRouter.render();await sleep(50);return;}
    await nav(hash);
  }
