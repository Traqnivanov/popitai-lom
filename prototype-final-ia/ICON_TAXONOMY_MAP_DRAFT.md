...
- `Почистване на дом`: приетата по-рано прахосмукачка е заменена с кофа, моп, един препарат и микрофибърна кърпа. Голямата професионална количка е точна за `Офиси и входове`, но не за дома.
- `Бани и плочки`, `Електро`: owner verdict `да ок са`.
- `Покриви`: първият вариант с мистрия е отхвърлен като семантично неточен; приет е коригираният покрив с комин, обшивка и отделна червена керемида.
- `Шпакловка / гипсокартон / боядисване`, `Дограма и врати`, `Отопление и климатици`: owner verdict `приемам`; комбинираните знаци трябва да останат четими като целия Master leaf при 56 px.
