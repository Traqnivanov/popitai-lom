(() => {
  ...
  function services(){return familyPage('Услуги','Търси по конкретна задача. Пълната taxonomy се показва след избора, не наведнъж.',serviceFamilies,{placeholder:'Напр. ВиК, почистване, автосервиз, хамали…',quick:['ВиК','Електро','Ремонти','Почистване','Автосервиз','Транспорт'],context:'Услуги',add:'#add/listing?category=Услуги',addLabel:'Предложи или потърси услуга'});}
  ...
  function auto(){return familyPage('Автомобили','МПС, части и услуги са един тематичен вход, но не един datastore.',autoGroups,{placeholder:'Напр. автомобил, гуми, диагностика…',quick:['Автомобили','Гуми','Части','Диагностика','Пътна помощ'],context:'Автомобили',notice:'<div class="notice">„Автомобилни услуги“ води към същите Service records от „Услуги“ — не се създава дублирана taxonomy.</div>',add:'#add/listing?category=Автомобили и МПС',addLabel:'Добави обява'});}
  ...
  function shops(){return familyPage('Магазини','Специализиран Shop owner с шест широки семейства и по-конкретни tags.',shopGroups,{placeholder:'Какъв магазин търсиш?',context:'Магазини',owner:'Shops',resultDetail:'firm',quick:shopGroups.slice(0,4),add:'#add/shop',addLabel:'Добави магазин',notice:'<div class="owner-box"><strong>Owner:</strong> Shops. Generic Listing/Firm flow не заобикаля специализирания Shop flow.</div>'});}
  ...
  function health(){return familyPage('Здраве и лекари','Частни практики през specialized Health flow; verified здравната информация остава в Инфо Лом.',healthGroups,{placeholder:'Лекар, специалист, стоматолог…',context:'Здраве и лекари',owner:'Health/Info',resultDetail:'health',quick:healthGroups,add:'#add/health',addLabel:'Добави лекар / здравна услуга',notice:'<div class="owner-box"><strong>Owner:</strong> Health/Info. Generic medical listings не се използват.</div>'});}
  ...
  function results(query){
    ...
    return `<div class="page">${pageHead(`Резултати: ${group}`,`${context} → ${group}. Това е prototype results layer между browse и detail.`,'Обяви и услуги')}<div class="shell"><div class="owner-box"><strong>Lifecycle owner:</strong> ${esc(owner)}. Discovery context-ът не създава копие на записа.</div><div class="result-list">${demoRow(`${group} — примерен резултат 1`,`Прототипен резултат от ${owner}.`,'Резултат',`#detail/${detailType}`,context)}${demoRow(`${group} — примерен резултат 2`,`Втори пример за list density и detail journey.`,'Резултат',`#detail/${detailType}`,context)}</div><div class="page-tools"><a class="btn primary" href="${addTarget}">＋ Публикувай в правилния flow</a><a class="btn" href="#add/question">Не намираш? Задай въпрос</a></div></div></div>`;
  }
  ...
  function formPage(kind,query){
    ...
    const prefillBox = prefill.length?`<div class="owner-box"><strong>Видим prefill:</strong> ${prefill.map(([k,v])=>`${esc(k)} = ${esc(v)}`).join(' · ')}. В production стойностите остават editable; при edit режим saved record има приоритет.</div>`:'';
