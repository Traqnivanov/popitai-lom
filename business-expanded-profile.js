...
(() => {
  ...
  function syncGalleryAndCover() {
    ...
    // Първата снимка е cover — скриваме я от галерията, is-featured на втората
    allImages.forEach((img, i) => {
      img.loading = i === 0 ? "eager" : "lazy";
      img.decoding = "async";
      img.removeAttribute("fetchpriority");
      if (i === 0) {
        const wrapper = img.closest("button, li, .gallery-item, figure, .media-gallery-item");
        if (wrapper) {
          wrapper.style.display = "none";
          wrapper.classList.remove("is-featured");
        } else img.style.display = "none";
      }
      if (i === 1) {
        const wrapper = img.closest("button, li, .gallery-item, figure, .media-gallery-item");
        if (wrapper) wrapper.classList.add("is-featured");
      }
    });
