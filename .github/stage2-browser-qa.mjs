...
try{
  ...
  await go('#home');
  const home=await page.evaluate(()=>({h1:document.querySelector('h1')?.textContent?.trim(),text:document.body.innerText}));
  ...
  const res=await page.evaluate(()=>({h1:document.querySelector('h1')?.textContent?.trim(),text:document.body.innerText}));
  ...
  const addState=await page.evaluate(()=>({text:document.body.innerText,category:document.querySelector('#listing-category')?.value,sub:document.querySelector('#listing-subcategory')?.value,type:document.querySelector('#listing-type')?.value}));
  ...
  const changed=await page.evaluate(()=>({sub:document.querySelector('#listing-subcategory')?.value,disabled:document.querySelector('#listing-subcategory')?.disabled,discovery:document.querySelector('[data-proto-form]')?.dataset.discoveryContext||'',contextHidden:document.querySelector('.service-context-summary')?.hidden,dirty:document.querySelector('[data-proto-form]')?.dataset.dirty}));
  ...
  check('Category change marks form dirty',changed.dirty==='true',changed.dirty||'');

  const dialogsBefore=dialogs.length;
  await go('#add/listing?category=%D0%A3%D1%81%D0%BB%D1%83%D0%B3%D0%B8&subcategory=%D0%92%D0%B8%D0%9A&type=%D0%94%D0%B0%D0%B2%D0%B0');
  check('Dirty navigation confirmation triggered',dialogs.length>dialogsBefore,dialogs.slice(dialogsBefore).join(','));
