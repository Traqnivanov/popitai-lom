...
(function(){
  ...
  const proposalConfigs = {
    ...
    internet:{title:"Добави доставчик",lead:"Предложи доставчик на интернет или телевизия за проверка.",back:"info/utilities",fields:()=>`${field("Име на доставчика","name",{required:true,min:2})}${field("Местен адрес (по желание)","address",{})}${field("Телефон (по желание)","phone",{type:"tel"})}${field("Какво предлага","services",{textarea:true,required:true,min:10,full:true})}${field("Източник / линк (по желание)","source",{full:true})}`},
  ...
  window.submitForm = async function submitFormWithInfoProposal(form){
    ...
    successScreen("Предложението е изпратено за проверка","Нищо не е публикувано автоматично. Съответният раздел в Инфо Лом трябва да потвърди данните преди публично показване.",back,"Към раздела");
  };
})();
