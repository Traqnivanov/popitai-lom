(() => {
  ...
  function submissionCard(x,entries){
    ...
    return `<article class="info-admin-card" data-submission-card="${esc(x.id)}">
      ...
      <div class="info-admin-actions"><button type="button" class="info-admin-approve" data-sub-review="${esc(x.id)}">${correction?"Прегледай и приложи":"Прегледай и одобри"}</button><button type="button" class="info-admin-reject" data-sub-reject="${esc(x.id)}">Отхвърли</button></div>
