(() => {
  ...
  function injectPendingSummary(pending){
    ...
    card.innerHTML=`<div><strong>Инфо Лом: ${pending} ${pending===1?"чакаща заявка":"чакащи заявки"}</strong><span>Има съдържание за администраторска проверка.</span></div><button type="button">Отвори Инфо Лом</button>`;
  ...
  function syncPendingUi(count){
    ...
    const waitingBox = document.querySelector('[data-admin-section="pending"], .admin-content');
  ...
  async function loadData(){
    const [e,s,r] = await Promise.all([
      ...
      client.from("info_submissions").select("id,category,subcategory,entry_type,submitted_by,data,status,admin_note,created_at,reviewed_at,reviewed_by").eq("status","pending").order("created_at",{ascending:false}),
      client.from("info_error_reports").select("id,entry_id,reported_by,category,subcategory,description,status,admin_note,created_at,reviewed_at,reviewed_by").eq("status","pending").order("created_at",{ascending:false})
    ]);
  ...
  async function insertHistory({entryId,fieldName,oldValue,newValue,reason,source}){
    const {error}=await client.from("info_entry_history").insert({
      ...
      reason:String(reason||"Администраторска промяна"),
      source:String(source||"Администраторска проверка")
    });
  ...
  function editableDataRows(data){
    ...
    return keys.map(key=>{
      ...
      if(key==="nzok"){
        ...
        return `<label>${esc(humanLabel(key))}
          <select data-edit-data-key="${esc(key)}" data-edit-nzok="1">
            <option value="yes"${selected==="yes"?" selected":""}>Да</option>
            <option value="no"${selected==="no"?" selected":""}>Не</option>
            <option value="unknown"${selected==="unknown"?" selected":""}>Не знам</option>
          </select>
        </label>`;
      }
      return `<label>${esc(humanLabel(key))}
        <textarea rows="${String(shown).length>90?4:2}" data-edit-data-key="${esc(key)}">${esc(shown)}</textarea>
      </label>`;
    }).join("");
  }

  function entryCard(x){
    return `<article class="info-admin-card" data-entry-card="${esc(x.id)}">
      <h3>${esc(x.name)}</h3>
      <div class="info-admin-meta">${esc(catLabel(x.category))} · ${esc(subLabel(x.subcategory))}</div>
      <div class="info-admin-meta">${esc(PUBLICATION_LABELS[x.publication_status] || x.publication_status)} · ${esc(RELIABILITY_LABELS[x.reliability_status] || x.reliability_status)}</div>
      <div class="info-admin-history">Последна промяна: ${esc(fmt(x.updated_at))}</div>
      <div class="info-admin-actions">
        <button type="button" class="info-admin-secondary" data-edit-entry="${esc(x.id)}">Редактирай</button>
        ${x.publication_status==="hidden"
          ? `<button type="button" class="info-admin-secondary" data-restore-entry="${esc(x.id)}">Върни</button>`
          : `<button type="button" class="info-admin-reject" data-remove-entry="${esc(x.id)}">Премахни</button>`}
      </div>
      <div class="info-admin-review" data-edit-panel="${esc(x.id)}" hidden>
        <div class="info-admin-grid">
          <label>Име<input data-edit-name value="${esc(x.name)}"></label>
          <label>Публичен статус
            <select data-edit-publication>
              <option value="published"${x.publication_status==="published"?" selected":""}>${esc(PUBLICATION_LABELS.published)}</option>
              <option value="review"${x.publication_status==="review"?" selected":""}>${esc(PUBLICATION_LABELS.review)}</option>
              <option value="hidden"${x.publication_status==="hidden"?" selected":""}>${esc(PUBLICATION_LABELS.hidden)}</option>
            </select>
          </label>
          <label>Надеждност<select data-edit-reliability>${reliabilityOptions(x.reliability_status||"strong")}</select></label>
          <label>Потвърден източник<input data-edit-source value="${esc(x.confirmed_source||"")}"></label>
        </div>
        <details class="info-admin-review" open>
          <summary><strong>Данни на записа</strong></summary>
          <div data-edit-data-fields>${editableDataRows(x.data)}</div>
        </details>
        <label>Причина за промяната *<input data-edit-reason placeholder="Напр. актуализиран телефон от официален източник"></label>
        <label>Бележка за потвърждението<textarea rows="2" data-edit-confirmation-note>${esc(x.confirmation_note||"")}</textarea></label>
        <div class="info-admin-box" data-edit-preview hidden></div>
        <div class="info-admin-actions">
          <button type="button" class="info-admin-secondary" data-preview-entry="${esc(x.id)}">Преглед на промените</button>
          <button type="button" class="info-admin-approve" data-save-entry="${esc(x.id)}">Запази промените</button>
        </div>
        <div class="info-admin-msg" data-msg></div>
      </div>
    </article>`;
  }
  ...
  function submissionCard(x,entries){
    ...
    return `<article class="info-admin-card" data-submission-card="${esc(x.id)}">
      <h3>${esc(correction?(d.name||"Предложена корекция"):(d.name||"Нов предложен запис"))}</h3>
      <div class="info-admin-meta">${esc(catLabel(x.category))} · ${esc(subLabel(x.subcategory))}</div>
      <div class="info-admin-meta">Подадено: ${esc(fmt(x.created_at))}</div>
      ${correction?`<div class="info-admin-box">Какво: ${esc(d.field_type||"друго")}\nСега: ${esc(d.current_problem||"")}\nПредложено: ${esc(d.proposed_value||"")}${d.source?`\nИзточник: ${esc(d.source)}`:""}${d.note?`\nБележка: ${esc(d.note)}`:""}</div>`:humanSubmissionFields(d)}
      <div class="info-admin-actions"><button type="button" class="info-admin-approve" data-sub-review="${esc(x.id)}">${correction?"Прегледай и приложи":"Прегледай и одобри"}</button><button type="button" class="info-admin-secondary" data-sub-return="${esc(x.id)}">Върни за корекция</button><button type="button" class="info-admin-reject" data-sub-reject="${esc(x.id)}">Отхвърли</button></div>
      <div class="info-admin-review" data-sub-panel="${esc(x.id)}" hidden>
      ${correction?`
        <div class="info-admin-grid"><label>Запис<select data-target-entry><option value="">Избери запис</option>${entries.filter(e=>e.category===x.category&&e.subcategory===x.subcategory).map(e=>`<option value="${esc(e.id)}"${selectedEntry?.id===e.id?' selected':''}>${esc(e.name)}</option>`).join("")}</select></label><label>Поле в data (JSON key)<input data-field-key value="${esc(["phone","address","working_hours"].includes(d.field_type)?d.field_type:"")}" placeholder="напр. phone, address, services"></label></div>
        <label>Нова стойност<textarea rows="3" data-new-value>${esc(d.proposed_value||"")}</textarea></label>
        <label>Причина<input data-reason value="${esc(d.current_problem||"Одобрена потребителска корекция")}"></label>
        <label>Източник<input data-source value="${esc(d.source||"")}" placeholder="Официален източник / проверка"></label>
        <label>Админ бележка<textarea rows="2" data-admin-note>${esc(d.note||"")}</textarea></label>
        <div class="info-admin-actions"><button type="button" class="info-admin-approve" data-apply-correction="${esc(x.id)}">Потвърди промяната</button></div>`:`
        <div class="info-admin-grid"><label>Публичен статус<select data-publication><option value="published">${esc(PUBLICATION_LABELS.published)}</option><option value="review">${esc(PUBLICATION_LABELS.review)}</option><option value="hidden">${esc(PUBLICATION_LABELS.hidden)}</option></select></label><label>Надеждност<select data-reliability>${reliabilityOptions("strong")}</select></label></div>
        <label>Потвърден източник *<input data-source value="${esc(d.source||"")}" placeholder="Официален източник / администраторска проверка"></label>
        <label>Админ бележка<textarea rows="2" data-admin-note>${esc(d.note||"")}</textarea></label>
        <div class="info-admin-actions"><button type="button" class="info-admin-approve" data-approve-add="${esc(x.id)}">Създай записа</button></div>`}
        <div class="info-admin-msg" data-msg></div>
      </div>
    </article>`;
  }
  ...
  async function open(){
    ...
    menu?.querySelectorAll("button").forEach(b=>b.classList.toggle("active",b.dataset.infoAdmin==="1"));
    if(!box)return;
  ...
  function render(state,box){
    ...
    const draw=tab=>{box.querySelectorAll("[data-tab]").forEach(b=>b.classList.toggle("active",b.dataset.tab===tab));const out=document.getElementById("info-admin-list");if(!out)return;if(tab==="entries"){
        out.innerHTML=`
          <div class="info-admin-toolbar">
            <input type="search" data-entry-search placeholder="Търси по име, адрес, телефон, раздел...">
            <select data-entry-category>
              <option value="">Всички раздели</option>
              ${[...new Set(entries.map(e=>e.category))].sort().map(v=>`<option value="${esc(v)}">${esc(catLabel(v))}</option>`).join("")}
            </select>
            <select data-entry-status>
              <option value="">Всички статуси</option>
              <option value="published">Публикувани</option>
              <option value="review">За проверка</option>
              <option value="hidden">Скрити</option>
            </select>
          </div>
          <div class="info-admin-meta" data-entry-count></div>
          <div data-entry-groups></div>`;
        renderEntryGroups(out,entries);
        wireEntryFilters(out,entries);
      }else if(tab==="subs"){out.innerHTML=`<div class="stack-list">${subs.map(x=>submissionCard(x,entries)).join("")||'<article class="empty-card"><p>Няма чакащи предложения.</p></article>'}</div>`;wireSubmissions(out,state);}else{out.innerHTML=`<div class="stack-list">${reports.map(x=>reportCard(x,entries)).join("")||'<article class="empty-card"><p>Няма чакащи сигнали.</p></article>'}</div>`;wireReports(out,state);}};
  ...
  function renderEntryGroups(root,allEntries){
    ...
    const filtered=allEntries.filter(e=>{
      ...
      if(status && e.publication_status!==status) return false;
      if(search && !entrySearchText(e).includes(search)) return false;
      return true;
    });

    const count=root.querySelector("[data-entry-count]");
    if(count) count.textContent=`Показани ${filtered.length} от ${allEntries.length} записа`;

    const groupsRoot=root.querySelector("[data-entry-groups]");
    if(!groupsRoot) return;

    if(!filtered.length){
      groupsRoot.innerHTML='<article class="empty-card"><p>Няма намерени записи.</p></article>';
      return;
    }
    ...
    groupsRoot.innerHTML=[...byCategory.entries()]
      .sort((a,b)=>catLabel(a[0]).localeCompare(catLabel(b[0]),"bg"))
      .map(([category,items])=>`
        <details class="info-admin-group" ${search||category===root.querySelector("[data-entry-category]")?.value?"open":""}>
          <summary><strong>${esc(catLabel(category))}</strong> <span>(${items.length})</span></summary>
          <div class="stack-list">${items.map(entryCard).join("")}</div>
        </details>
      `).join("");
  ...
  async function removeEntrySafely(entry,root){
    ...
    const second=window.confirm(`Наистина ли искате да премахнете „${entry.name}“ от публичния сайт?`);
    if(!second) return;

    try{
      const oldStatus=entry.publication_status;
      const {error}=await client.from("info_entries").update({
      ...
      await insertHistory({
        ...
        oldValue:oldStatus,
        newValue:"hidden",
        reason:"Премахнато от публичния сайт от администратор",
        source:entry.confirmed_source||"Администраторска промяна"
      });

      await open();
    }catch(err){
      console.error(err);
      window.alert("Неуспешно премахване. Проверете записа преди повторен опит.");
    }
  }

  async function restoreEntry(entry){
    const ok=window.confirm(`Да се върне „${entry.name}“ като публикуван запис?`);
    if(!ok) return;
    try{
      const {error}=await client.from("info_entries").update({
      ...
      await insertHistory({
        ...
        oldValue:"hidden",
        newValue:"published",
        reason:"Върнат в публичния сайт от администратор",
        source:entry.confirmed_source||"Администраторска промяна"
      });
      ...
      console.error(err);
      window.alert("Неуспешно връщане на записа.");
    }
  }
  ...
  function parseEditedValue(text,original){
    ...
    if(Array.isArray(original)||(original&&typeof original==="object")){
      try{return raw?JSON.parse(raw):(Array.isArray(original)?[]:{});}
      catch{throw new Error("Невалиден формат в едно от сложните полета.");}
    }
    if(typeof original==="boolean"){
      if(raw==="true"||raw==="Да")return true;
      if(raw==="false"||raw==="Не")return false;
    }
  ...
  function collectEntryChanges(panel,entry){
    const changes=[];
    const next={
      name:panel.querySelector("[data-edit-name]")?.value.trim()||"",
      publication_status:panel.querySelector("[data-edit-publication]")?.value||entry.publication_status,
      reliability_status:panel.querySelector("[data-edit-reliability]")?.value||entry.reliability_status,
      confirmed_source:panel.querySelector("[data-edit-source]")?.value.trim()||"",
      confirmation_note:panel.querySelector("[data-edit-confirmation-note]")?.value.trim()||"",
      data:{...(entry.data||{})}
    };
    ...
    add("publication_status",entry.publication_status,next.publication_status);
    add("reliability_status",entry.reliability_status,next.reliability_status);
    add("confirmed_source",entry.confirmed_source||"",next.confirmed_source);
    add("confirmation_note",entry.confirmation_note||"",next.confirmation_note);
    panel.querySelectorAll("[data-edit-data-key]").forEach(el=>{
      const key=el.dataset.editDataKey;
      const oldValue=entry.data?.[key];
      let newValue;
      if(key==="nzok"&&el.dataset.editNzok==="1"){
        if(el.value==="yes") newValue=true;
        else if(el.value==="no") newValue=false;
        else newValue=undefined;
      }else{
        newValue=parseEditedValue(el.value,oldValue);
      }
      if(JSON.stringify(oldValue)!==JSON.stringify(newValue)){
        if(newValue===undefined) delete next.data[key];
        else next.data[key]=newValue;
        changes.push({field:`data.${key}`,old:oldValue,new:newValue});
      }
    });
    return {next,changes};
  }
  ...
  function renderChangePreview(panel,changes){
    const box=panel.querySelector("[data-edit-preview]");
    if(!box)return;
    box.hidden=false;
    if(!changes.length){box.innerHTML="<strong>Няма промени.</strong>";return;}
    box.innerHTML=`<strong>Ще се променят ${changes.length} полета:</strong>`+changes.map(c=>`<div style="margin-top:8px"><b>${esc(c.field)}</b><br>Сега: ${esc(typeof c.old==="object"?JSON.stringify(c.old):c.old??"(празно)")}<br>Ново: ${esc(typeof c.new==="object"?JSON.stringify(c.new):c.new??"(празно)")}</div>`).join("");
  }

  async function saveEntryEdit(entry,panel){
    const reason=panel.querySelector("[data-edit-reason]")?.value.trim()||"";
    if(!reason){panelMsg(panel,"Попълнете причина за промяната.");return;}
    let collected;
    try{collected=collectEntryChanges(panel,entry);}catch(err){panelMsg(panel,err.message||"Невалидна стойност.");return;}
    const {next,changes}=collected;
    if(!changes.length){panelMsg(panel,"Няма направени промени.");return;}
    if(!next.name){panelMsg(panel,"Името не може да е празно.");return;}
    if(!next.confirmed_source){panelMsg(panel,"Попълнете потвърден източник.");return;}
    renderChangePreview(panel,changes);
    const summary=changes.map(c=>`${c.field}: ${typeof c.old==="object"?JSON.stringify(c.old):c.old??"(празно)"} → ${typeof c.new==="object"?JSON.stringify(c.new):c.new??"(празно)"}`).join("\n");
    if(!window.confirm(`Да се запишат тези промени директно в Supabase?\n\n${summary}`))return;
    try{
      const {error}=await client.from("info_entries").update({
        name:next.name,
        data:next.data,
        publication_status:next.publication_status,
        reliability_status:next.reliability_status,
        confirmed_source:next.confirmed_source,
        confirmation_note:next.confirmation_note,
        confirmed_at:nowIso(),
        updated_by:currentUser?.id||null
      }).eq("id",entry.id);
      if(error)throw error;
      for(const c of changes){
        await insertHistory({
          entryId:entry.id,
          fieldName:c.field,
          oldValue:c.old==null?null:(typeof c.old==="string"?c.old:JSON.stringify(c.old)),
          newValue:c.new==null?null:(typeof c.new==="string"?c.new:JSON.stringify(c.new)),
          reason,
          source:next.confirmed_source
        });
      }
      panelMsg(panel,"Промените са записани директно в Supabase и в историята.",true);
      setTimeout(open,500);
    }catch(err){
      console.error(err);
      panelMsg(panel,"Грешка при записването. Проверете данните преди повторен опит.");
    }
  }
  ...
  function wireEntries(root,state){
    root.querySelectorAll("[data-edit-entry]").forEach(btn=>btn.addEventListener("click",()=>{
      const panel=root.querySelector(`[data-edit-panel="${CSS.escape(btn.dataset.editEntry)}"]`);
      if(panel)panel.hidden=!panel.hidden;
    }));
    root.querySelectorAll("[data-preview-entry]").forEach(btn=>btn.addEventListener("click",()=>{
      const entry=state.entries.find(e=>e.id===btn.dataset.previewEntry);
      const panel=btn.closest("[data-edit-panel]");
      if(!entry||!panel)return;
      try{renderChangePreview(panel,collectEntryChanges(panel,entry).changes);}catch(err){panelMsg(panel,err.message||"Невалидна стойност.");}
    }));
    root.querySelectorAll("[data-save-entry]").forEach(btn=>btn.addEventListener("click",()=>{
      const entry=state.entries.find(e=>e.id===btn.dataset.saveEntry);
      const panel=btn.closest("[data-edit-panel]");
      if(entry&&panel)saveEntryEdit(entry,panel);
    }));
    root.querySelectorAll("[data-remove-entry]").forEach(btn=>btn.addEventListener("click",()=>{
      const entry=state.entries.find(e=>e.id===btn.dataset.removeEntry);
      if(entry) removeEntrySafely(entry,root);
    }));
    root.querySelectorAll("[data-restore-entry]").forEach(btn=>btn.addEventListener("click",()=>{
      const entry=state.entries.find(e=>e.id===btn.dataset.restoreEntry);
      if(entry) restoreEntry(entry);
    }));
    root.querySelectorAll("[data-edit-panel]").forEach(panel=>{
      panel.querySelectorAll("input,textarea,select").forEach(el=>{
        el.addEventListener("input",()=>clearPanelMessage(panel));
        el.addEventListener("change",()=>clearPanelMessage(panel));
      });
    });
  }
...
function wireSubmissions(root,state){
  ...
  function wireReports(root,state){
    ...
    root.querySelectorAll("[data-report-return]").forEach(btn=>btn.addEventListener("click",async()=>{
      const x=state.reports.find(r=>r.id===btn.dataset.reportReturn);
      if(!x)return;
      const note=window.prompt("Каква допълнителна информация е нужна?","")?.trim();
      if(!note)return;
      try{
        await markReport(x.id,"needs_info",note);
        window.alert("Поискана е допълнителна информация от подателя.");
        await open();
      }catch(err){
        console.error(err);
        window.alert("Неуспешно връщане към подателя.");
      }
    }));
  ...
  document.addEventListener("DOMContentLoaded",async()=>{
    ...
    if(m){
      new MutationObserver(()=>ensureButton()).observe(m,{childList:true});
    }
